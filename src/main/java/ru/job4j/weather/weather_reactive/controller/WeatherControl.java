package ru.job4j.weather.weather_reactive.controller;
...
public class WeatherControl {
    ...
    @GetMapping(value = "/cityGreatThen/{c}", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<Weather> greater(@PathVariable Integer c) {
        Flux<Weather> data = weathers.findGreatThen(c);
        Flux<Long> delay = Flux.interval(Duration.ofSeconds(3));
        return Flux.zip(data, delay).map(Tuple2::getT1);
    }
}